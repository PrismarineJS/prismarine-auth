...
class MinecraftJavaTokenManager {
  ...
  * @returns {object}
  */
  async fetchEntitlements (accessToken: string) {
  ...
  async fetchCertificates (accessToken: string) {
    ...
    return {
      profileKeys: {
        ...
        public: crypto.createPublicKey({ key: toDER(cert.keyPair.publicKey), format: 'der', type: 'spki' }),
        private: crypto.createPrivateKey({ key: toDER(cert.keyPair.privateKey), format: 'der', type: 'pkcs8' })
      }
    };
  }
  ...
  async reportPlayerChat (report: {
    ...
    const reportedMessagesCount = report.messages.reduce((acc, cur) => { acc += (cur.reported ? 1 : 0); return acc }, 0)
    ...
    if (report.messages.length > reportLimits.maxEvidenceMessageCount) throw Error(`Too many messages provided as evidence, max allowed is ${reportLimits.maxEvidenceMessageCount}`)
